...
static int decode_rxmraw(raw_t *raw)
{
    ...
    if (week==0) {
        rtktrace(3,"ubx rxmraw week=0 error: len=%d nsat=%d\n",raw->len,nsat);
        return 0;
    }
    /* time tag adjustment */
    if (tadj>0.0) {
...
static int decode_rxmrawx(raw_t *raw)
{
    ...
    if (week==0) {
        rtktrace(3,"ubx rxmrawx week=0 error: len=%d nsat=%d\n",raw->len,nsat);
        return 0;
    }
    if (raw->outtype) {
...
static int decode_enav(raw_t *raw, int sat, int off)
{
    ...
    /* test even-odd parts */
    if (part1!=0||part2!=1) {
        rtktrace(2,"ubx rawsfrbx gal page even/odd error: sat=%2d\n",sat);
        return -1;
    }
    ...
    if (rtk_crc24q(crc_buff,25)!=getbitu(buff+16,82,24)) {
        rtktrace(2,"ubx rawsfrbx gal page crc error: sat=%2d\n",sat);
        return -1;
    }
    ...
    if (eph.sat!=sat) {
        rtktrace(2,"ubx rawsfrbx gal svid error: sat=%2d %2d\n",sat,eph.sat);
        return -1;
    }
    if (!strstr(raw->opt,"-EPHALL")) {
