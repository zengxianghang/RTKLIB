...
static int eph_supports_code(int sys, int type, unsigned char code)
{
    ...
    if (sys==SYS_GAL) {
        ...
        return 0; /* no V1 E6 broadcast code-bias model yet */
    }
    if (sys==SYS_CMP) {
...
static const geph_t *select_signal_geph(gtime_t time, int sat, const nav_t *nav)
{
                                ...
                                rtklib_signal_bias_info_ext_t *info)
{
    ...
    if (sys==SYS_GLO) {
        ...
        else return 0; /* G3 has no V1 broadcast code-bias correction */
        if (info) {
            info->system=sys;
    ...
    if (sys==SYS_GPS||sys==SYS_QZS) {
        ...
        if (type==NAV_D1D2||type==NAV_D1||type==NAV_D2) {
            if (code==CODE_L2I) bias=CLIGHT*eph->tgd[0]; /* B1I: +c*TGD1 in raw P */
            else if (code==CODE_L7I) bias=CLIGHT*eph->tgd[1]; /* B2I: +c*TGD2 */
            else if (code==CODE_L6I) bias=0.0; /* B3I clock reference */
            else return 0;
        }
        else if (type==NAV_CNV1||type==NAV_CNV2) {
            if (code==CODE_L1P) bias=CLIGHT*eph->tgd[0]; /* B1C pilot */
            else if (code==CODE_L5P) bias=CLIGHT*eph->tgd[1]; /* B2a pilot */
            else return 0;
        }
        else if (type==NAV_CNV3&&code==CODE_L7D) {
            bias=CLIGHT*eph->tgd[0]; /* B2b data */
        }
        else return 0;
    }
    else return 0;
                              ...
                              rtklib_signal_bias_info_ext_t *bias_info)
{
    ...
    if (!ionocorr(obs->time,nav,obs->sat,pos,azel,opt->ionoopt,&dion,&vion)) return 0;
    dion*=SQR(wavelength_m/lam_carr[0]);
    if (!tropcorr(obs->time,nav,pos,azel,opt->tropopt,&dtrp,&vtrp)) return 0;

    p=obs->P[0]-code_bias;
    *residual_m=p-(r+receiver_clock_bias_m-CLIGHT*dts[0]+dion+dtrp)-receiver_system_bias_m;
    if (azel_rad) {
        azel_rad[0]=azel[0];
        azel_rad[1]=azel[1];
    }
                             ...
                             double *residual_mps, double azel_rad[2])
{
    ...
    for (j=0;j<3;j++) relative_velocity[j]=rs[j+3]-receiver_velocity_ecef_mps[j];
