...
static int decode_type1046(rtcm_t *rtcm)
{
    ...
    if (i+492<=rtcm->len*8) {
        ...
        rtktrace(2,"rtcm3 1046 length error: len=%d\n",rtcm->len);
        return -1;
    }
    rtktrace(4,"decode_type1046: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    ...
    if (!(sat=satno(sys,prn))) {
        rtktrace(2,"rtcm3 1046 satellite number error: prn=%d\n",prn);
        return -1;
    }
    eph.sat=sat;
    eph.week=week+1024; /* gal-week = gst-week + 1024 */
...
static int decode_type1042(rtcm_t *rtcm)
{
    ...
    if (i+499<=rtcm->len*8) {
        ...
        rtktrace(2,"rtcm3 1042 length error: len=%d\n",rtcm->len);
        return -1;
    }
    rtktrace(4,"decode_type1042: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    ...
    if (!(sat=satno(sys,prn))) {
        rtktrace(2,"rtcm3 1042 satellite number error: prn=%d\n",prn);
        return -1;
    }
    eph.sat=sat;
